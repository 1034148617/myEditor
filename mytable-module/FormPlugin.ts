...
function deleteHandler(newEditor: IDomEditor): boolean {
    ...
    const [cellNodeEntry] = Editor.nodes(newEditor, {
        match: n => DomEditor.checkNodeType(n, 'tab-cell'),
    })
    if (cellNodeEntry) {
...
function withMyTable<T extends IDomEditor>(editor: T): T {
    const {
        insertBreak,
        deleteBackward,
        deleteForward,
    ...
    newEditor.insertBreak = () => {
        const selectedNode = DomEditor.getSelectedNodeByType(newEditor, 'ptab')
    ...
    newEditor.handleTab = () => {
        ...
        if (selectedNode) {
            ...
            if (DomEditor.checkNodeType(above[0], 'tab-cell')) {
            ...
            if (next) {
                ...
                if (DomEditor.checkNodeType(topLevelNodes[topLevelNodesLength - 1], 'ptab')) {
    ...
    newEditor.normalizeNode = ([node, path]) => {
        ...
        if (SlateElement.isElement(prevNode)) {
            ...
            if (prevNodeType === 'ptab' || newEditor.isVoid(prevNode)) {
                const p = genEmptyParagraph()
                Transforms.insertNodes(newEditor, p, {
                    at: path, // 在表格前面插入
                })
    ...
    newEditor.insertData = (data: DataTransfer) => {
        const tableNode = DomEditor.getSelectedNodeByType(newEditor, 'ptab')
        if (tableNode == null) {
    ...
    //     const cell = DomEditor.getSelectedNodeByType(newEditor, 'tab-cell')
    //     if (cell == null) {
    //         selectAll()
    //         return
    //     }
...
export default withMyTable
Boot.registerPlugin(withMyTable)

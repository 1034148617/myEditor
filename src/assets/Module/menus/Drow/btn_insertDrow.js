...
export function genPdrNode(group, Did, isTemplate, content) {
    return {
        type: 'pdr',
        group: group,
        Did: Did,
...
export function insert_pdr_template(editor, group, content) {
    ...
    if (SlateNode.string(node) === "") {
        ...
        const PdrNode = genPdrNode(group, Utils.NewID(32), true, content);
        Transforms.insertNodes(editor, PdrNode);
    } else {
        const selectedText = SlateNode.string(node); // 当前行文字
        const PdrNode = genPdrNode(group, Utils.NewID(32), true, selectedText);

        SlateTransforms.setNodes(editor, PdrNode, {
            at: [row]
        });
    }
...
export const InsertDrowConf = {
    key: 'menu-insert-pdr',
    factory() {
        return new InsertDrow()
    },
}
